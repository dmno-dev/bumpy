...
async function setupPat(rootDir: string, repo: string): Promise<void> {
  ...
  p.note(
    [
      ...
      '',
      `3. Under ${pc.bold('Repository access')}, select ${pc.bold('"Only select repositories"')}`,
      `   and choose ${pc.cyan(repo)}`,
      '',
      `4. Under ${pc.bold('Permissions → Repository permissions')}, grant:`,
...
function printPatWorkflowSnippet(): void {
  p.note(
    [
      ...
      `    ${pc.bold('BUMPY_GH_TOKEN: ${{ secrets.BUMPY_GH_TOKEN }}')}`,
    ].join('\n'),
...
function printAppWorkflowSnippet(): void {
  p.note(
    [
      ...
      '  env:',
      '    GH_TOKEN: ${{ github.token }}',
      `    ${pc.bold('BUMPY_GH_TOKEN: ${{ steps.app-token.outputs.token }}')}`,
    ].join('\n'),
    'Update your workflow',
  );
}
