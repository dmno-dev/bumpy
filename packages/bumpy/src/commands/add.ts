...
export async function addCommand(rootDir: string, opts: AddOptions): Promise<void> {
  ...
  if (opts.packages) {
    ...
    const selected = await multiSelect<string>(
      ...
      [...pkgs.values()].map((p) => ({ label: `${p.name} (${p.version})`, value: p.name })),
    );
    ...
    for (const name of selected) {
      const bumpType = await select<BumpTypeWithIsolated>(`Bump type for ${colorize(name, 'cyan')}:`, BUMP_CHOICES);
        ...
        if (dependents.length > 0 || cascadeTargets) {
          ...
          if (wantCascade) {
            ...
            if (cascadeSelected.length > 0) {
              const cascadeBump = await select<BumpType>('Cascade bump type:', [
                { label: 'patch', value: 'patch' },
                { label: 'minor', value: 'minor' },
                { label: 'major', value: 'major' },
              ]);
  ...
  for (const r of releases) {
    const cascade =
      'cascade' in r && Object.keys(r.cascade).length > 0
        ? ` (cascade: ${Object.entries(r.cascade)
            .map(([k, v]) => `${k}:${v}`)
            .join(', ')})`
        : '';
