...
describe("isPackageManaged", () => {
  ...
  test("private packages managed when privatePackages.version is true", () => {
    expect(isPackageManaged("pkg-a", true, makeConfig({
  ...
  test("ignore supports globs", () => {
    expect(isPackageManaged("@myorg/internal-tool", false, makeConfig({
      ignore: ["@myorg/internal-*"],
    }))).toBe(false);
    expect(isPackageManaged("@myorg/core", false, makeConfig({
      ignore: ["@myorg/internal-*"],
    }))).toBe(true);
  });

  test("include overrides private", () => {
    expect(isPackageManaged("my-vscode-ext", true, makeConfig({
      include: ["my-vscode-ext"],
    }))).toBe(true);
  });

  test("include supports globs", () => {
    expect(isPackageManaged("@myorg/app-a", true, makeConfig({
      include: ["@myorg/app-*"],
    }))).toBe(true);
    expect(isPackageManaged("@myorg/lib-b", true, makeConfig({
      include: ["@myorg/app-*"],
    }))).toBe(false);
  });

  test("include overrides ignore", () => {
    expect(isPackageManaged("pkg-special", false, makeConfig({
      ignore: ["pkg-*"],
      include: ["pkg-special"],
    }))).toBe(true);
    // other pkg-* still ignored
    expect(isPackageManaged("pkg-other", false, makeConfig({
      ignore: ["pkg-*"],
      include: ["pkg-special"],
    }))).toBe(false);
  });
  ...
  test("per-package managed: true overrides everything", () => {
    ...
    expect(isPackageManaged("pkg-a", true, makeConfig({
      ignore: ["pkg-a"],
    }), { managed: true })).toBe(true);
  });

  test("per-package managed: false overrides everything", () => {
    // Public + included, but managed: false
    expect(isPackageManaged("pkg-a", false, makeConfig({
      include: ["pkg-a"],
    }), { managed: false })).toBe(false);
  });
});
