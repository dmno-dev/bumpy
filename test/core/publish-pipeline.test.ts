...
describe("publishPackages", () => {
  ...
  test("dry run does not modify files", async () => {
    ...
    packages.set("my-pkg", makePkg("my-pkg", "1.0.0", pkgDir, {
  ...
  test("custom publish command gets version/name templated", async () => {
    ...
    const packages = new Map<string, WorkspacePackage>();
    packages.set("my-ext", makePkg("my-ext", "2.0.0", pkgDir, {
      bumpy: {
  ...
  test("skips private packages without custom publish", async () => {
    ...
    const packages = new Map<string, WorkspacePackage>();
    packages.set("private-pkg", makePkg("private-pkg", "1.0.0", pkgDir, {
      private: true,
    }));
  ...
  test("workspace protocol resolution for publish", async () => {
    ...
    packages.set("app", makePkg("app", "1.0.1", appDir, {
      dependencies: { core: "workspace:^" },
    }));
    ...
    const plan: ReleasePlan = {
      changesets: [],
      releases: [
        { name: "core", type: "minor", oldVersion: "1.0.0", newVersion: "1.1.0", changesets: [], isDependencyBump: false, isCascadeBump: false },
        { name: "app", type: "patch", oldVersion: "1.0.0", newVersion: "1.0.1", changesets: [], isDependencyBump: true, isCascadeBump: false },
      ],
    };
  ...
  test("catalog: protocol resolution for publish", async () => {
    ...
    const plan: ReleasePlan = {
      changesets: [],
      releases: [
        { name: "app", type: "patch", oldVersion: "1.0.0", newVersion: "1.0.1", changesets: [], isDependencyBump: false, isCascadeBump: false },
      ],
    };
